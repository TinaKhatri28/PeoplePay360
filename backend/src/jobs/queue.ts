...
export const dispatchJob = async (
  ...
  if (payrollQueue && getIsRedisAvailable()) {
    try {
      await payrollQueue.add(jobName, data, {
        attempts: 3,
      ...
      logger.warn(`Failed to push job to BullMQ: ${err.message}. Falling back to in-process execution.`);
    }
  }
