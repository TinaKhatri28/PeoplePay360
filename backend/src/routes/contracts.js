...
function nextRef() {
  ...
  const count = db.prepare("SELECT COUNT(*) c FROM contracts WHERE ref LIKE ?").get(`CON/${year}/%`).c;
...
router.post('/', requireRole('HR Manager', 'HR Payroll User'), (req, res) => {
  const b = req.body;
  if (!b.employee_id || !b.start_date || !b.wage) {
    return res.status(400).json({ error: 'employee_id, start_date and wage are required' });
  }

  // Business rule: don't allow two Running contracts overlapping for the same employee
  if ((b.status || 'Running') === 'Running') {
    const existing = db.prepare(`
      SELECT * FROM contracts WHERE employee_id = ? AND status = 'Running'
      AND (end_date IS NULL OR end_date >= ?)
    `).all(b.employee_id, b.start_date);
    if (existing.length) {
      // auto-expire the previous running contract as of the day before the new one starts
  ...
  const info = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(ref, b.employee_id, b.start_date, b.end_date || null, b.wage, b.status || 'Running',
...
router.put('/:id', requireRole('HR Manager', 'HR Payroll User'), (req, res) => {
  const b = req.body;
  db.prepare(`
