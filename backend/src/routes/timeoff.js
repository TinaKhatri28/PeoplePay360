...
// ---- Types ----
router.get('/types', (req, res) => res.json(db.prepare('SELECT * FROM time_off_types').all()));
router.post('/types', requireRole('HR Manager'), (req, res) => {
...
router.post('/allocations', requireRole('HR Manager'), (req, res) => {
...
router.post('/requests', (req, res) => {
  ...
  const alloc = db.prepare('SELECT * FROM time_off_allocations WHERE employee_id = ? AND type_id = ?').get(eid, type_id);
  const type = db.prepare('SELECT * FROM time_off_types WHERE id = ?').get(type_id);
  ...
  res.status(201).json({ id: info.lastInsertRowid, duration: days });
});

router.post('/requests/:id/approve', requireRole('HR Manager'), (req, res) => {
  ...
  const alloc = db.prepare('SELECT * FROM time_off_allocations WHERE employee_id = ? AND type_id = ?')
    .get(request.employee_id, request.type_id);
  if (alloc) {
...
router.post('/requests/:id/refuse', requireRole('HR Manager'), (req, res) => {
