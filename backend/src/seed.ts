...
import db from './db';
import { computeEmployeePayslip } from './payrollEngine';
...
const tables = [
  'payslips', 'payruns', 'salary_rules', 'salary_structures', 'time_off_requests',
...
for (const name of deptNames) {
  ...
  dept[name] = Number(info.lastInsertRowid);
}

// Working schedules
function mkSchedule(name: string, company: string, days: Array<{ day: string; start: string; end: string; breakHours: number }>): number {
  const info = db.prepare('INSERT INTO working_schedules (name, company, days_per_week, schedule_json) VALUES (?,?,?,?)')
    .run(name, company, days.length, JSON.stringify(days));
...
function mkEmployee(
  ...
  `).run(
    name, email, '+91 98' + Math.floor(10000000 + Math.random() * 89999999), position, departmentId,
    managerId, scheduleId, bank, initials(name)
  );
  return Number(info.lastInsertRowid);
}
...
const structId = Number(db.prepare('INSERT INTO salary_structures (name) VALUES (?)').run('Employee Salary').lastInsertRowid);
const rule = (
  name: string,
  category: string,
  method: string,
  extra: { amount?: number; percentage?: number; percentage_of?: string; formula_key?: string },
  seq: number
) => db.prepare(`
...
rule('Basic Salary', 'Basic', 'FIXED', {}, 10);
...
function mkContract(ref: string, empId: number, start: string, end: string | null, wage: number, status: string, department: string, position: string, scheduleId: number) {
...
// Attendance for Sept 2026
const allEmployees = [aarav, sara, john, neha, priya, rohan];
function addAttendance(empId: number, day: number, checkInHour: number | null, checkOutHour: number | null, status: string) {
...
for (let day = 1; day <= 4; day++) {
  addAttendance(aarav, day, 9, day === 3 ? 19 : 18, 'Present');
...
// Time off types
const potId = Number(db.prepare("INSERT INTO time_off_types (name, unit, allocation_required, approval_role) VALUES ('Paid Time Off','Days',1,'Manager')").run().lastInsertRowid);
const sickId = Number(db.prepare("INSERT INTO time_off_types (name, unit, allocation_required, approval_role) VALUES ('Sick Leave','Days',0,'Manager')").run().lastInsertRowid);
const compId = Number(db.prepare("INSERT INTO time_off_types (name, unit, allocation_required, approval_role) VALUES ('Comp Off','Hours',1,'Officer')").run().lastInsertRowid);
const unpaidId = Number(db.prepare("INSERT INTO time_off_types (name, unit, allocation_required, approval_role) VALUES ('Unpaid Leave','Days',0,'Manager')").run().lastInsertRowid);

// Allocations
function allocate(empId: number, typeId: number, allocated: number, taken: number, approver: number) {
...
// Users
function mkUser(email: string, password: string, roles: string[], employeeId: number) {
...
// Payruns
function runPayrunForMonth(year: number, month: number, employeeIds: number[], status: string): number {
  const runId = Number(db.prepare('INSERT INTO payruns (period_month, period_year, structure_id, company, status) VALUES (?,?,?,\'OXP Pvt Ltd\',?)')
...
function computeAndFinalize(runId: number, year: number, month: number, status: string) {
  const slips = db.prepare('SELECT * FROM payslips WHERE payrun_id = ?').all(runId) as any[];
